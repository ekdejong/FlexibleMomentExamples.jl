...
for j in 1:length(size_distribution_list)
    ...
    MK.lines!(ax2, sol.t, r_l)

    # results - spectra
    if DSD == "monodisperse"
        dist_new = CPD.update_dist_from_moments(dist_init[1], tuple(sol[5:end, end]...))
        xm = dist_new.θ
        rm = (xm / 1000 * 3 / 4 / π) .^ (1 / 3) * 1e6
        Mm = dist_new.θ * dist_new.n
        MK.lines!(ax3m, [rm, rm], [0.0, Mm], color = MK.Cycled(j + 1), label = "final") # arbitrary maximum in the case of monodisperse
    else
