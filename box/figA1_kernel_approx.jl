...
for i = 1:n
    for j = 1:n
        ...
        z_tensor2[i, j] = compute_kernel_from_tensor_matrix(
            matrix_of_kernels,
...
cb1 = Colorbar(fig[1, 2], colorrange = (min_lim1, max_lim1), label = L"K \, (m^3/s)")
...
cb2 = Colorbar(fig[1, 4], colorrange = (min_lim1, max_lim1), label = L"K \, (m^3/s)")
...
cb3 = Colorbar(fig[2, 2], colorrange = (min_lim2, max_lim2), label = L"K \, (m^3/s)")
...
cb1 = Colorbar(fig[2, 4], colorrange = (min_lim2, max_lim2), label = L"K \, (m^3/s)")
...
# Save the figure
save(
    joinpath(dirname(@__DIR__), "figures/figA1/figA1_KernelFunction_Approximation.pdf"),
    fig,
    pt_per_unit = 2,
)
