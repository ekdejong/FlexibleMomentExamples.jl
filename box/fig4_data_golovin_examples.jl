"Box model with variable number of modes, fixed thresholds, & Golovin kernel"
...
function init_conditions_gamma(init_moments, num_modes)
    if num_modes > 1
        thresholds = 10 .^ range(-9, -5; length=num_modes)
        thresholds[end] = Inf
...
for nd in n_dist_list
    ...
    box_output(sol, ODE_parameters, "results/fig4_box_$(nd)_gamma_golovin.nc", FT)
end